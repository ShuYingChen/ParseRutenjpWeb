...
public class WebContentParser {
    ...
    public List<Attribute> getCartJson(List<Attribute> attributeList, Document doc, List<ExtraBlock> extra, List<List<String>> groupList) {
    ...
    public List<Attribute> getAdditionalAttributes(List<Attribute> attributeList, Elements tableBlock, int lastIndex) {
    ...
    public PageInfo getGlobalInfo(Elements doc) {
        ...
        Elements pictureElements = pictures.select("div");
        if (pictureElements.size() > 1) {
    ...
    public ParseData parse(String awsDtm, String content, Charset charset) {
        ParseData result = null;

        try {
            String webContent = new String(new BASE64Decoder().decodeBuffer(content), charset);
            ...
            Elements tableBlockList = doc.select("div#rakutenLimitedId_aroundCart").
                    select("td.floating-cart-sku-table");
            Elements cartBlockList = doc.select("div#rakutenLimitedId_aroundCart").select("table").select("tbody").select("tr");
            ...
            result = new ParseData(pageInfo.getCode(), jsonStr);
    ...
    public static void main(String[] args) {
        WebContentParser parser = new WebContentParser();
//        ParseData data = parser.parse("",",")
    }
}